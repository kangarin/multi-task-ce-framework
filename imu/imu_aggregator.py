...
class ImuAggregator(Aggregator):
    ...
    def run(self):
        self.subscriber.subscribe(self._incoming_mq_topic, 
                                  callback=(lambda client, userdata, message:(
                                      self.lock.acquire(), 
                                      self.local_task_queue.append(ImuTask.deserialize(json.loads(message.payload.decode()))), 
                                      self.lock.release())),
                                      qos=2                                     
                                  )
    ...
    def insert_result(self, task: ImuTask):
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Imu aggregator')
    ...
    aggregator = ImuAggregator(f'aggregator_{id}', f'$share/python/testapp/aggregator_{id}', { 'window_size': 8 })
    aggregator.run()
