...
class VideoGeneratorFree(Generator):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--total_tasks', type=int, help='total tasks')
    id = parser.parse_args().id
    data_source = parser.parse_args().data_source
    total_tasks = parser.parse_args().total_tasks
    ...
    generator = VideoGeneratorFree(data_source, f'video_generator_free_{id}',
                               'testapp/video_generator', 0, {"frames_per_task": 5, "skipping_frame_interval": 5}, total_tasks)
    generator.run()
